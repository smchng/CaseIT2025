...
export const NavListMobile = ({
  ...
  useEffect(() => {
    // Close submenus whenever the path changes
    setSubOpen(null);
  }, [activePathname]);
    ...
    <ul className={listClassName}>
      {linkArray.map((item, index) => (
        <li
          key={index}
          className={twMerge(
            `${linkClassName} ${activePathname === item.ref && "text-black"} w-fit`,
            item?.className 
          )}
        >
          <div
          ...
          >
            {item.sublinks ? (
              <div
                onClick={() =>
                  setSubOpen(subOpen === item.ref ? null : item.ref)
                }
            ...
            <div
              className={`transition-transform ease-in-out-[cubic-bezier(.16,1,.3,1)] sm:max-w-[25rem] duration-500 bg-white w-screen sm:w-[70vw] px-16 py-12 h-screen absolute top-0 right-0 ${
                subOpen === item.ref ? "" : "translate-x-[100%]"
              }`}
            >
              <div
                onClick={() => {
              ...
              {subOpen === item.ref &&
                item.sublinks && ( // Check if this item's sublinks should be shown
